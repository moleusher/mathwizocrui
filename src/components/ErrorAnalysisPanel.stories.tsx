...
export const LongCause: Story = {
  args: {
    analysis: {
      cause: '学生在解这道题时出现了多处错误。首先，在第一步设未知数时没有明确写出单位，导致后续计算中单位混淆。其次，在代入公式时误将 $a=3$ 写成了 $a=2$，使得整个计算过程偏离了正确方向。最后，在计算结果时没有进行验算，导致错误没有被及时发现。',
      type: '计算失误',
    } as ErrorAnalysis,
  },
};
...
export const Collapsed: Story = {
  args: {
    analysis: {
      ...
      type: '知识点漏洞',
      suggestion: '建议复习配方法的步骤：$ax^2 + bx + c = a(x + \\frac{b}{2a})^2 + \\frac{4ac - b^2}{4a}$',
    } as ErrorAnalysis,
